...
*/          
	...
	function _tomainclient($template, $input) {
		global $session;
		...
		$ret = '../clients/clientNotes.php';
		$session->set( 'returnTo', $ret );
		$cl = $template->getClient(  );
		$clCode = $cl->getKeyValue(  );
	...
	function _doactualupdate($template, $input) {
		...
		if ($exists) {
			...
			$template->setSequence( $note->get( 'noClientSequence' ) );
	...
	$clientNotesTemplate = &$session->get( 'clientNotesTemplate' );

	if ($clientNotesTemplate == null) {
		$clientNotesTemplate = new ClientNotesTemplate( 'clientNotes.html' );
		$clientNotesTemplate->setProcess( '_doUpdate', 'updateNote' );
		$clientNotesTemplate->setProcess( '_goBack', 'back' );
		$clientNotesTemplate->setProcess( '_toMainClient', 'clientDetails' );
	}


	if (isset( $_GET['client'] )) {
		$clCode = $_GET['client'];
		$clientNotesTemplate->setClient( $clCode );
	}


	if (isset( $_GET['clientNote'] )) {
		$noCode = $_GET['clientNote'];
		$clientNotesTemplate->selectNoteToDisplay( $noCode );
	}


	if (( isset( $returnTo ) && $returnTo != null )) {
		$clientNotesTemplate->setReturnTo( $returnTo );
	}

	$session->set( 'clientNotesTemplate', $clientNotesTemplate );
	$clientNotesTemplate->process(  );
	$session->set( 'clientNotesTemplate', $clientNotesTemplate );
?>