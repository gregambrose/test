...
*/          
	...
	class outstandingmessagestemplate {
		function outstandingmessagestemplate($html) {
			ftemplate::ftemplate( $html );
			$this->addField( 'itemType' );
			$this->addField( 'originator' );
			$this->addField( 'lastUpdatedBy' );
			$this->addField( 'nextActionBy' );
			$this->set( 'itemType', 'A' );
		...
		function listitems($text) {
			...
			if (( $itemType == 'D' || $itemType == 'A' )) {
				$mangDocmType = MANAGEMENT_DOCM_TYPE;
				$q = '' . 'SELECT doCode, doWhenOriginated
				FROM documents
			   WHERE doDeleted != 1 AND doLocked != 1 AND doDocmType != ' . $mangDocmType;
