 ...
 */
	...
	function update_is_available($lv) {
		if (is_numeric( $lv )) {
			...
			return ( $script_parts[0] < $lv_parts[0] || ( $lv_parts[0] == $script_parts[0] && $script_parts[1] < $lv_parts[1] ) );
		} 
else {
			return null;
		}
	...
	function remote_file_contents($url) {
		...
		if ($remote_file_opening) {
			...
			if (extension_loaded( 'curl' )) {
				...
				$timing_out = (400 <= $info['http_code'] ? 1 : 0);
				curl_close( $ch );
	...
	function php_version() {
		$v = explode( '.', '5.3.4' );
		return array( 'major' => $v[0], 'minor' => $v[1], 'release' => $v[2] );
	}
	...
	function is_supported_php_version() {
		$v = php_version(  );
		return ( ( ( $v['major'] == 4 && 1 <= $v['minor'] ) || ( $v['major'] == 5 && ( 1 <= $v['minor'] || 3 <= $v['release'] ) ) ) || $v['major'] == 7 );
	}

	function is_php_version_or_greater($major, $minor, $release = 0) {
		$version = php_version(  );
		return ( ( $major < $version['major'] || ( $version['major'] == $major && $minor < $version['minor'] ) ) || ( ( $version['major'] == $major && $version['minor'] == $minor ) && $release <= $version['release'] ) );
	}
	...
	function match_arch_pattern($str) {
		$arch = null;
		$arch_patterns = array( 'i.?86' => 'x86', 'x86[-_]64' => 'x86', 'x86' => 'x86', 'amd64' => 'x86', 'SMP Tue Jan 01 00:00:00 CEST 2000 all GNU\/Linux' => 'x86', 'ppc64' => 'ppc', 'ppc' => 'ppc', 'powerpc' => 'ppc', 'sparc' => 'sparc', 'sun' => 'sparc', 'armv7l' => 'armv7l', 'aarch64' => 'aarch64' );
		foreach ($arch_patterns as $a) {
			$token = ;
	...
	function uname($part = 'a') {
		...
		if (!function_is_disabled( 'php_uname' )) {
			...
			if (( function_exists( 'posix_uname' ) && !function_is_disabled( 'posix_uname' ) )) {
	...
	function required_loader($unamestr = '') {
		...
		$php_major_version = substr( '5.3.4', 0, 3 );
		...
		return array( 'uname' => $un, 'arch' => $arch, 'oscode' => $os_code, 'oscode_h' => $os_code_h, 'osname' => $os_name, 'osnamequal' => $os_name_qual, 'osvariants' => $os_variants, 'osver' => $os_ver, 'osver2' => $os_ver_parts, 'file' => $file, 'file_ts' => $file_ts, 'wordsize' => $wordsize );
	}
...
' : '</tr>');
		...
		return array( 'THREAD_SAFE' => $thread_safe, 'DEBUG_BUILD' => $debug_build, 'PHP_INI' => $php_ini_path, 'PHP_INI_BASENAME' => $php_ini_basename, 'PHP_INI_DIR' => $php_ini_dir, 'PHP_INI_ADDITIONAL' => $php_ini_add, 'PHPRC' => getenv( 'PHPRC' ), 'CGI_CLI' => $cgi_cli, 'IS_CGI' => $is_cgi, 'IS_CLI' => $is_cli, 'PHP_COMPILER' => $php_compiler, 'SUPPORTED_COMPILER' => $is_supported_compiler, 'FULL_SS' => $ss['full'], 'SS' => $ss['short'] );
	}
	...
	function is_local() {
		...
		if ($_SERVER['SERVER_NAME'] == 'localhost') {
			...
			if (strpos( ':', $ip_address ) === false) {
				...
				$ret = ( ( $ip_parts[0] == 10 || ( ( $ip_parts[0] == 172 && 16 <= $ip_parts[1] ) && $ip_parts[1] <= 31 ) ) || ( $ip_parts[0] == 192 && $ip_parts[1] == 168 ) );
			} 
else {
				$ret = ( $ip_address == '::1' || ( $ip_address[0] == 'f' && ( 'c' <= $ip_address[1] && $ip_address[1] <= 'f' ) ) );
			}
		}
			...
			if ($type_must_be_chosen) {
				...
				if (( isset( $_SESSION['server_type'] ) && $_SESSION['server_type'] != SERVER_UNKNOWN )) {
					...
					if (is_local(  )) {
						...
						if (!is_possibly_dedicated_or_local(  )) {
	...
	function get_default_loader_dir_webspace() {
		return $_SERVER['DOCUMENT_ROOT'] . '\' . LOADER_SUBDIR;
	}
	...
	function get_loader_location_from_ini($php_ini = '') {
		...
		if ($loader_path === '') {
			...
			if (!@file_exists( $loader_path )) {
				...
				if (basename( $loader_path ) == $loader_path) {
...
';
		...
		foreach ($unrec_ini_files as $urinif) {
			$output .=  . ';;; *UNRECOGNISED INI FILE at ' . $urinif . '* ;;;' . '
';
	...
	function scan_inis_for_loader() {
		...
		foreach ($ini_file_list as $f) {
			...
			if (( $shared_server && array_key_exists( ERROR_INI_NOT_FOUND, $ldloc['errors'] ) )) {
				...
				if (array_key_exists( ERROR_INI_NOT_FOUND, $ldloc['errors'] )) {
		...
		if (!empty( $$ini_files_not_found )) {
			$plural = (1 < count( $ini_files_not_found ) ? 's' : '');
	...
	function find_loader_filesystem() {
		...
		$loader_name = get_loader_name(  );
		$suggested_loader_path = $ld_inst_dir . '\' . $loader_name;
	...
	function ioncube_loader_version_information() {
		...
		if (function_exists( 'ioncube_loader_iversion' )) {
			...
			if (( ( $lat_parts[0] < $cur_parts[0] || ( $cur_parts[0] == $lat_parts[0] && $lat_parts[1] < $cur_parts[1] ) ) || ( ( $cur_parts[0] == $lat_parts[0] && $cur_parts[1] == $lat_parts[1] ) && $lat_parts[2] <= $cur_parts[2] ) )) {
				$old_version = false;
			} 
else {
	...
	function runtime_loader_location() {
		...
		if ($ext_path !== false) {
			...
			$i = strlen( $rd );
			$loader_loc = '\' . basename( $here ) . '\' . get_loader_name(  );
	...
	function runtime_loading_errors() {
		...
		if (false === $ext_path) {
			...
			$expected_file = dirname( __FILE__ ) . '\' . get_loader_name(  );
	...
	function zend_extension_lines($loader_dir) {
		...
		if (( !is_bool( $sysinfo['THREAD_SAFE'] ) || !$sysinfo['THREAD_SAFE'] )) {
			$path = $qt . $loader_dir . '\' . $loader['file'] . $qt;
		...
		if (( ( !is_bool( $sysinfo['THREAD_SAFE'] ) && !is_php_version_or_greater( 5, 3 ) ) || $sysinfo['THREAD_SAFE'] )) {
			...
			$path = $qt . $loader_dir . '\' . $loader['file_ts'] . $qt;
	...
	function zend_extension_instructions($server_type, $loader_dir) {
		...
		if (( SERVER_SHARED == $server_type && own_php_ini_possible(  ) )) {
			...
			$php_ini_path = $ini_dir . '\' . $php_ini_name;
...
' );
	...
	function php_ini_install($server_type_desc = null, $server_type = , $required = true) {
		$php_ini_name = ini_file_name(  );
		$default = get_default_address(  );
		echo '<p>';
	...
	function system_info_temporary_files() {
		...
		if (( ( empty( $$tmpfname_ini ) || empty( $$tmpfname_pinf ) ) || empty( $$tmpfname_add ) )) {
			...
			return array( 'ini' => $tmpfname_ini, 'phpinfo' => $tmpfname_pinf, 'additional' => $tmpfname_add );
		}
	...
	function system_info_archive_page() {
		...
		if (!empty( $$all_files )) {
			...
			if (extension_loaded( 'zip' )) {
				...
				if (( extension_loaded( 'zlib' ) && !is_ms_windows(  ) )) {
	...
	function loader_system($loader_location) {
		...
		if (!empty( $$loader_strs )) {
			if (preg_match( '/ioncube_loader_..?\.._(.)\.(.)\.(..?)(_nonts)?(_amd64)?\.dll/i', $loader_strs, &$version_matches )) {
				...
				if (( $loader_system['php_version_major'] == 7 && 2 <= $loader_system['php_version_minor'] )) {
					...
					if (( $loader_system['php_version_major'] == 7 && $loader_system['php_version_minor'] < 2 )) {
						...
						if (( $loader_system['php_version_major'] == 5 && 5 <= $loader_system['php_version_minor'] )) {
			...
			if (preg_match( '/ionCube Loader Version\s+(\S+)/', $loader_strs, &$loader_version )) {
				...
				if (preg_match( '/ioncube_loader_(\d{1,2}\.\d\.\d{1,2})\./', $loader_strs, &$loader_version )) {
					$loader_system['loader_version'] = $loader_version[1];
				} 
else {
	...
	function loader_compatibility_test($loader_location) {
		...
		if (preg_match( '/php version:\s*(.)\.(.)\.(..?)(-ts)?/i', $loader_strs, &$version_matches )) {
			...
			if (( ( ( is_bool( $sysinfo['THREAD_SAFE'] ) && $sysinfo['THREAD_SAFE'] ) && !is_ms_windows(  ) ) && !( isset( $version_matches[4] ) && $version_matches[4] == '-ts' ) )) {
				...
				if (( ( isset( $version_matches[4] ) && $version_matches[4] == '-ts' ) && !( is_bool( $sysinfo['THREAD_SAFE'] ) && $sysinfo['THREAD_SAFE'] ) )) {
			...
			if (preg_match( '/ioncube_loader_..?\.._(.)\.(.)\.(..?)(_nonts)?(_amd64)?\.dll/i', $loader_strs, &$version_matches )) {
				if (!is_ms_windows(  )) {
					...
					if (( ( ( isset( $version_matches[4] ) && $version_matches[4] == '_nonts' ) && is_bool( $sysinfo['THREAD_SAFE'] ) ) && $sysinfo['THREAD_SAFE'] )) {
						...
						if (( !( is_bool( $sysinfo['THREAD_SAFE'] ) && $sysinfo['THREAD_SAFE'] ) && !( isset( $version_matches[4] ) && $version_matches[4] == '_nonts' ) )) {
					...
					if (( $version_matches[1] == 7 && 2 <= $version_matches[2] )) {
						...
						if ($version_matches[1] == 7) {
							...
							if (( $version_matches[1] == 5 && 5 <= $version_matches[2] )) {
...
' );
		...
		if (!isset( $_SESSION['CREATED'] )) {
			...
			if (SESSION_LIFETIME_MINUTES * 60 < time(  ) - $_SESSION['CREATED']) {
				clear_session(  );
			}
		}
...
' );
	...
	function wizardversion_page() {
		...
		if (false === $wizard_version) {
			...
			if (update_is_available( $wizard_version )) {
	...
	function default_page($loader_extension = ) {
		$self = get_self(  );
	...
	function ioncube_24_is_available() {
		$loaderinfo = get_loaderinfo(  );
		$php_ver = php_version(  );
		return ( $loaderinfo['oscode'] == 'lin' && ( ( $php_ver['major'] == 5 && 3 <= $php_ver['minor'] ) || 5 < $php_ver['major'] ) );
	}
	...
	function loader_not_installed() {
		...
		if (!is_supported_php_version(  )) {
			echo '<p>Your server is running PHP version ' . '5.3.4' . ' and is
                unsupported by ionCube Loaders.  Recommended PHP 4 versions are PHP 4.2 or higher, 
                and PHP 5.1 or higher for PHP 5.</p>';
		} 
else {
			if ($latest_supported_php_version = is_after_max_php_version_supported(  )) {
				echo '<strong>Your server is running PHP version ' . '5.3.4' . ' and is
	...
	function server_selection_form() {
		...
		$hostprovider = htmlspecialchars( $hostprovider, 3, 'UTF-8' );
		$hosturl = (!empty( $_SESSION['hosturl'] ) ? $_SESSION['hosturl'] : '');
		$hosturl = htmlspecialchars( $hosturl, 3, 'UTF-8' );
		$form =  . '    <p>This Wizard will give you information on how to install the ionCube Loader.</p>
	...
	function loader_check_page($ext_name = ) {
		heading(  );
		...
		if (is_string( $loader_location )) {
			...
			if (empty( $$loader_errors )) {
				...
				if (!empty( $ini_loader['location'] )) {
					...
					$std_ld_path = $std_dir . '\' . get_loader_name(  );
				...
				foreach ($ini_files as $full_ini_loc) {
					...
					if (!ini_same_dir_as_wizard(  )) {
						...
						if (( ( ( !@file_exists( $shared_ini_file ) && !empty( $$shared_ini_loc ) ) && $ini_loc_dir != $shared_ini_loc ) && $ini_dir != $shared_ini_loc )) {
							...
							$here_ini_file = $herepath . '\' . $ini_name;
		...
		if (!empty( $$download )) {
			...
			if (empty( $$ini_file_name )) {
				...
				if (( ( !preg_match( '`^.*\.ini$`', $ini_file_name ) || preg_match( '`/`', $ini_file_name ) ) || preg_match( '`\\`', $ini_file_name ) )) {
					exit(  . 'Illegal file name ' . $ini_file_name );
				}
			}
...
';
		...
		if (( ( empty( $$ini_file_name ) || empty( $sys['PHP_INI_DIR'] ) ) || $sys['PHP_INI_BASENAME'] == $ini_file_name )) {
			...
			$original_ini_file = $sys['PHP_INI_DIR'] . '\' . $ini_file_name;
		}
	...
	function make_archive_list($basename, $archives_list = array(  ), $download_server = ) {
	...
	function error($m) {
		exit(  . '<b>ERROR:</b> <span class="error">' . $m . '</span><p>Please help us improve this script by <a href="' . SUPPORT_SITE . '">reporting this error</a> and including the URL to the script so that we can test it.' );
	}

	function filter_server_input($server_var) {
		$res = htmlspecialchars( $_SERVER[$server_var], 3, 'UTF-8' );
		return $res;
	}
	...
	function footer($update_info = null) {
		...
		$wizard_version_string =  . '    <script type="text/javascript">
    var xmlhttp;
...
';
	...
	define( 'IONCUBE_IP_ADDRESS', '94.101.154.134' );
	define( 'IONCUBE_ACCESS_ADDRESS', 'lwaccess.ioncube.com' );
	define( 'LOADERS_PAGE', 'https://loaders.ioncube.com/' );
	define( 'SUPPORT_SITE', 'https://support.ioncube.com/' );
	define( 'WIZARD_SUPPORT_TICKET_DEPARTMENT', '3' );
	define( 'LOADER_FORUM_URL', 'https://forum.ioncube.com/viewforum.php?f=4' );
	define( 'LOADERS_FAQ_URL', 'https://www.ioncube.com/faqs/loaders.php' );
	define( 'UNIX_ERRORS_URL', 'https://www.ioncube.com/loaders/unix_startup_errors.php' );
	define( 'LOADER_WIZARD_URL', LOADERS_PAGE );
	define( 'ENCODER_URL', 'https://www.ioncube.com/sa_encoder.php' );
	define( 'LOADER_VERSION_URL', 'https://www.ioncube.com/feeds/product_info/versions.php' );
	define( 'WIZARD_LATEST_VERSION_URL', LOADER_VERSION_URL . '?item=loader-wizard' );
	define( 'PHP_COMPILERS_URL', LOADER_VERSION_URL . '?item=php-compilers' );
	define( 'LOADER_PLATFORM_URL', LOADER_VERSION_URL . '?item=loader-platforms-all' );
	define( 'LOADER_LATEST_VERSIONS_URL', LOADER_VERSION_URL . '?item=loader-versions' );
	define( 'LOADER_PHP_VERSION_URL', LOADER_VERSION_URL . '?item=loader-php-support' );
	define( 'WIZARD_STATS_URL', 'https://www.ioncube.com/feeds/stats/wizard.php' );
	define( 'IONCUBE_DOWNLOADS_SERVER', 'https://downloads.ioncube.com/loader_downloads' );
	define( 'IONCUBE24_URL', 'https://ioncube24.com' );
