...
*/          
	...
	class struct_book {
		...
		var $draw_sheet_name = true;
		var $bg_color = 'white';
		var $month_names = null;
		var $month_names_short = null;
		...
		function _check_palette() {
			if (0 < count( $this->_palette )) {
				return null;
			}

			$this->_palette = array( '000000', 'ffffff', 'ff0000', '00ff00', '0000ff', 'ffff00', 'ff00ff', '00ffff', '800000', '008000', '000080', '808000', '800080', '008080', 'c0c0c0', '808080', '9999ff', '993366', 'ffffcc', 'ccffff', '660066', 'ff8080', '0066cc', 'ccccff', '000080', 'ff00ff', 'ffff00', '00ffff', '800080', '800000', '008080', '0000ff', '00ccff', 'ccffff', 'ccffcc', 'ffff99', '99ccff', 'ff99cc', 'cc99ff', 'ffcc99', '3366ff', '33cccc', '99cc00', 'ffcc00', 'ff9900', 'ff6600', '666699', '969696', '003366', '339966', '003300', '333300', '993300', '993366', '333399', '333333' );
		}
...
';
		...
		function get_css_font($idx) {
			...
			if (!$this->overr_fnt_underline) {
				if ($fnt['underline']) {
					$stack .= 'text-decoration: underline;';
				}
			}
	...
	class struct_sheet extends struct_book {
		...
		function get_array() {
			...
			while ($r <= $this->num_rows) {
				...
				while ($c <= $this->num_cols) {
					$data = ' ';
					$options = '';
					$cur_type = (isset( $this->table[$r][$c]['type'] ) ? $this->table[$r][$c]['type'] : 0 - 2);

					if (( isset( $this->table[$r][$c]['data'] ) || $cur_type == 0 - 1 )) {
						switch ($cur_type) {
							case 0 - 1: {
								$data = ' ';
								break;
					...
					if (( ( ( isset( $this->table[$r][$c]['data'] ) && isset( $this->_parent->_cell_data['unicode'][$this->table[$r][$c]['data']] ) ) && $this->_parent->_cell_data['unicode'][$this->table[$r][$c]['data']] ) && !is_numeric( $data ) )) {
						$data = unicode2ascii( $data );
					}
				...
				while ($c <= $this->num_cols) {
					...
					if (( isset( $this->table[$r][$c]['data'] ) || $cur_type == 0 - 1 )) {
						switch ($cur_type) {
							...
							case 1: {
								if ($this->_parent->overr_int_align) {
									$options .= ' class=xls_int';
								}
							...
							case 2: {
								if ($this->_parent->overr_double_align) {
									$options .= ' class=xls_dbl';
								}
					...
					if (( ( isset( $this->_parent->_xf['fg_color'] ) && isset( $this->table[$r][$c]['xf'] ) ) && isset( $this->_parent->_palette[$this->_parent->_xf['fg_color'][$this->table[$r][$c]['xf']]] ) )) {
					...
					if (( 2 < strlen( $bg_color ) && !$this->_parent->overr_bgcolor )) {
					...
					if (( isset( $this->_col_width[$c] ) && !$this->_parent->overr_width )) {
...
';
						...
						while ($sr <= $merge_end_row) {
							$sc = $merge_start_col;

							while ($sc <= $merge_end_col) {
...
';
...
', trim( strrev( chunk_split( strrev( $i ), 3 ) ) ) ) );
			switch (strlen( $f )) {
				case 1: {
					$f .= '0';
					break;
		...
		function _convert_date($date, $format) {
			...
			if ($date_ar['hours'] < 10) {
				$date_ar['hours'] = '0' . $date_ar['hours'];
			}

			$date_ar['hours_12'] = (12 < $f_h ? $f_h - 12 : $f_h);
			$date_ar['am_pm'] = (12 <= $f_h ? 'PM' : 'AM');
			$date_ar['minutes'] = $f_m;

			if ($date_ar['minutes'] < 10) {
				$date_ar['minutes'] = '0' . $date_ar['minutes'];
			}

			$date_ar['seconds'] = $f_s;

			if ($date_ar['seconds'] < 10) {
				$date_ar['seconds'] = '0' . $date_ar['seconds'];
			}
			...
			$format = preg_replace( '/\//', '.', $format );
			$format = preg_replace( '/[;@]/', '', $format );
			$srch = array( '\'a\'i', '\'p\'i', '\'m\'i', '\'y\'i', '\'d\'i', '\'h\'i', '\'s\'i' );
			$rpls = array( '%a%', '%p%', '%m%', '%y%', '%d%', '%h%', '%s%' );
			$format = preg_replace( $srch, $rpls, $format );
			$srch = array( '\'%A%%M%\.%P%%M%\'i', '\'\:%m%%m%\'i', '\'\:%m%\'i', '\'%m%%m%%m%%m%\'i', '\'%m%%m%%m%\'i', '\'%m%%m%\'i', '\'%m%\'i', '\'%y%%y%%y%%y%\'i', '\'%y%%y%\'i', '\'%d%%d%%d%%d%\'i', '\'%d%%d%\'i', '\'%d%\'i', '\'%h%%h%\'i', '\'%h%\'i', '\'%s%%s%\'i', '\'%s%\'i' );
			$rpls = array( $date_ar['am_pm'], ':' . $date_ar['minutes'], ':' . (int)$date_ar['minutes'], $this->_parent->month_names[(int)$date_ar['month'] - 1], $this->_parent->month_names_short[(int)$date_ar['month'] - 1], $date_ar['month'], $date_ar['month'], $date_ar['year'], substr( $date_ar['year'], 0 - 2 ), $date_ar['day'], $date_ar['day'], (int)$date_ar['day'], $date_ar['hours'], $date_ar['hours_12'], $date_ar['seconds'], (int)$date_ar['seconds'] );
			...
			function _check_palette() {
				switch () {
					case 1: {
						...
						array( '000000', 'ffffff', 'ff0000', '00ff00', '0000ff', 'ffff00', 'ff00ff', '00ffff', '800000', '008000', '000080', '808000', '800080', '008080', 'c0c0c0', '808080', '9999ff', '993366', 'ffffcc', 'ccffff', '660066', 'ff8080', '0066cc', 'ccccff', '000080', 'ff00ff' );
					}
				}

				$this->_palette = array( 'ffff00', '00ffff', '800080', '800000', '008080', '0000ff', '00ccff', 'ccffff', 'ccffcc', 'ffff99', '99ccff', 'ff99cc', 'cc99ff', 'ffcc99', '3366ff', '33cccc', '99cc00', 'ffcc00', 'ff9900', 'ff6600', '666699', '969696', '003366', '339966', '003300', '333300', '993300', '993366', '333399', '333333' );
			}
