...
*/          
	...
	function _doactualupdate($template, $input) {
		...
		$document->set( 'doInscoAttachedTo', $detailAttachedTo );
		...
		if ($exists) {
			...
			$template->setSequence( $document->get( 'doInscoSequence' ) );
	...
	$inscoDocmsTemplate = &$session->get( 'inscoDocmsTemplate' );

	if ($inscoDocmsTemplate == null) {
		$inscoDocmsTemplate = new InscoDocmsTemplate( 'inscoDocms.html' );
		$inscoDocmsTemplate->setProcess( '_doUpdate', 'updateDocm' );
		$inscoDocmsTemplate->setProcess( '_goBack', 'back' );
	}


	if (isset( $_GET['insco'] )) {
		$icCode = $_GET['insco'];
		$inscoDocmsTemplate->setInsco( $icCode );
	}


	if (isset( $_GET['inscoDocument'] )) {
		$doCode = $_GET['inscoDocument'];
		$inscoDocmsTemplate->selectDocumentToDisplay( $doCode );
	}


	if (( isset( $returnTo ) && $returnTo != null )) {
		$inscoDocmsTemplate->setReturnTo( $returnTo );
	}

	$session->set( 'inscoDocmsTemplate', $inscoDocmsTemplate );
	$inscoDocmsTemplate->process(  );
	$session->set( 'inscoDocmsTemplate', $inscoDocmsTemplate );
?>